...
### Maintaining automated dependency updates
...
Update `python-versions` in an existing entry when its dependency locks need a
different Python version. Keep each value quoted so YAML does not interpret the
version as a number.
