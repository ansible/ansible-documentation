...
ansible_connection
    Connection type to the host. This can be the name of any Ansible connection plugin. SSH protocol types are ``ssh`` or ``paramiko``. The default is ssh. Non-SSH based types are described in the next section.
