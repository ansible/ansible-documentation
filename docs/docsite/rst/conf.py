...
applied_tags_count = sum(int(tags.has(known_tag_name)) for known_tag_name in KNOWN_TAGS)
assert (
    applied_tags_count == 1
), "Exactly one of the following tags expected: {tags}".format(
...
VERSION = (
    ...
    "devel"
    if tags.has("core_lang") or tags.has("core")
    else
    # Controls branch version for Ansible package releases
    "devel" if tags.has("ansible") or tags.has("all") else "<UNKNOWN>"
)
AUTHOR = "Ansible, Inc"
...
extensions = [
    ...
    "sphinx_copybutton",
    "sphinx_reredirects",  # redirects pages that have moved or been removed
]
...
project = (
    "Ansible Core"
    if (tags.has("all") or tags.has("core_lang") or tags.has("core"))
    else "Ansible" if tags.has("ansible") else "<UNKNOWN>"
)
...
html_context = {
    ...
    "latest_version": (
        ...
        if tags.has("all")
            ...
            "2.18"
            if tags.has("core_lang") or tags.has("core")
            else "11" if tags.has("ansible") else "<UNKNOWN>"
        )
    ),
    ...
    "available_versions": (
        ...
        if tags.has("all")
            ...
            if tags.has("core_lang")
                ...
                if tags.has("core")
                else ("latest", "2.9", "devel") if tags.has("ansible") else "<UNKNOWN>"
            )
        )
    ),
}
...
html_title = (
    ...
    if (tags.has("all") or tags.has("core_lang") or tags.has("core"))
    else "Ansible Community Documentation" if tags.has("ansible") else "<UNKNOWN>"
)
...
redirects = {}
if tags.has("core"):
    redirects_file = Path("./_data/pages.yaml")
    if redirects_file.exists():
        with redirects_file.open(mode="r", encoding="utf-8") as f:
            redirects = yaml.safe_load(f)
elif tags.has("ansible"):
    data_dir = Path("./_data")
    if data_dir.exists():
        redirects = {}
