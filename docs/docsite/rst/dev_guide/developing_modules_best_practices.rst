...
* You must always use ``module.run_command``, over Python methods like, but not limted to: ``subprocess``, ``Popen`` and ``os.system``.
...
* When displaying a URL, normally or as part of a warning or error, use the ``mask_url`` function to avoid disclosing possible inline user and password information.
* Sensitive values marked with ``no_log=True`` will automatically flag this value as a ``secret`` and strip it from Ansible outputs.  If your module could return these sensitive values as part of a dictionary key name, you should call the ``ansible.module_utils.basic.sanitize_keys()`` function to strip the values from the keys. See the ``uri`` module for an example.
