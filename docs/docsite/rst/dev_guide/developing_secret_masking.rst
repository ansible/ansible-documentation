...
* The ``result`` mapping of every task result handed to a callback plugin.
...
.. note::
   Masking works by searching output for the exact registered string. See :ref:`playbooks_secret_masking` for the minimum and maximum secret lengths and the rules that apply to short, overlapping, and adjacent secrets. If you know a secret can also appear in output in another shape or format, register that form as well so it is redacted too. A base64 encoded or URL encoded copy of a registered secret is a different string and is not masked on its own. The JSON-escaped form of a secret is the one exception. Because most Ansible output and module data is JSON, the registry derives the escaped forms of each secret itself, with and without ``\uXXXX`` escapes for non-ASCII characters, and masks those as well.
...
The ``result`` mapping of every task result handed to a callback plugin is masked before the plugin sees it. Ansible walks ``CallbackTaskResult.result`` and replaces registered secrets at any depth, including inside loop results, so a callback never receives an unmasked secret value in the result. The walk covers:

* **Strings.** Every ``str`` value and every ``str`` dictionary key is passed through ``mask_secrets()``. When two keys mask to the same placeholder, a numeric suffix is appended to keep both entries, for example ``$REDACTED$`` and ``$REDACTED$ (2)``.
* **Numbers.** An ``int`` or ``float`` whose text form is a registered secret is replaced with the placeholder string, so ``{"pin": 1234567}`` becomes ``{"pin": "$REDACTED$"}`` and the result stays valid when serialized. A callback that expects a number in a result must handle receiving a string instead.
* **Command output lines.** The ``stdout_lines`` and ``stderr_lines`` values are rebuilt from the masked ``stdout`` and ``stderr`` text, so a secret that spans several lines is masked in the lines as well. This only happens when the lines value still matches the split text. A module that sets ``stdout_lines`` itself has each line masked on its own like any other list.

Booleans, bytes, and ``None`` are returned unchanged. Only ``result.result`` is masked. Anything else the callback can reach is not:

* The task and play names, and the ``task``, ``host``, ``warnings``, ``deprecations``, and ``exception`` attributes of the result.
* The statistics passed to ``v2_playbook_on_stats()`` and the arguments of any other callback method.
* Data the callback builds by combining or deriving values, for example a value it decodes, hashes, or re-encodes after receiving the result.

Output sent through ``Display()`` is masked automatically wherever the data came from. Output written anywhere else, such as a file, a socket, an HTTP request, or a database, should be passed through ``mask_secrets()`` first so that data from outside ``result.result`` is masked as well. A callback that only writes through ``Display()`` needs no changes. The ``junit`` and ``tree`` callbacks shipped with ``ansible-core`` write to files and call ``mask_secrets()`` on everything they write, which also covers the task and play names in their output:
...
.. code-block:: python
    ...
    class CallbackModule(CallbackBase):
        ...
        def v2_runner_on_ok(self, result):
            # result.result is already masked, but the task name is not, so mask the
            # serialized form before writing it.
            entry = {'task': result.task_name, 'host': result.host.name, 'result': result.result}
            entry_json = mask_secrets(json.dumps(entry, default=str))

            with open('/var/log/ansible-results.jsonl', 'a') as fd:
                fd.write(entry_json + '\n')

The ``mask_secrets()`` function masks a secret in its literal form and in its JSON-escaped form, so a callback can serialize data with ``json.dumps()`` first and mask the resulting text, as the example above does. A secret that contains a double quote, a backslash, a control character, or any other character that is written differently when encoded as JSON is still recognized as a secret:
...
Masking the serialized text is not always enough, or not what you want, for data that does not come from ``result.result``. Mask each value before serializing it instead when:
...
All of the :ref:`limitations of secret masking <secret_masking_limitations>` described in the user guide apply to the masked result and to any output a callback masks itself, so read them before designing how your callback handles results.
