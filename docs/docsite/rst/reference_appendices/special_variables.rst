...
.. glossary::
    ...
    inventory_hostname_short
        The short version of `inventory_hostname`, it is the first section after spliting it via ``.``.
        So for the ``inventory_hostname`` of ``www.example.com``, the ``inventory_hostname_short`` would be ``www``.
